...
def run_ctier_pipeline(input_dir: Path | str) -> Path:
    ...
    print(f"{RUN_ID}=== Stage 3: normalization ===", flush=True)
    normalized_path = run_normalizer(semantic_path)
    print(f"{RUN_ID}=== Stage 4: classification ===", flush=True)
    classified_path = run_classifier(normalized_path)
    print(f"{RUN_ID}=== Pipeline complete. Final output: {classified_path}", flush=True)
